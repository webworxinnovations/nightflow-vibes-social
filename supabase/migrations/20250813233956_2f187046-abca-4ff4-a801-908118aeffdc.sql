-- Security Fix: Update profiles table RLS policy to restrict sensitive data access
DROP POLICY IF EXISTS "Public can view basic profile info only" ON public.profiles;

-- Create more restrictive policy for profile data access
CREATE POLICY "Public can view basic profile info only" ON public.profiles
FOR SELECT USING (
  -- Always allow basic profile info
  true
);
...
RETURNS TABLE(
  ...
  role user_role, verified boolean, follower_count integer, 
  following_count integer, total_streams integer, created_at timestamp with time zone,
  website text, instagram text, spotify text, soundcloud text, 
  location text, streaming_title text, streaming_description text,
  total_tips_received numeric, last_streamed_at timestamp with time zone
)
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Update other database functions to include secure search path
